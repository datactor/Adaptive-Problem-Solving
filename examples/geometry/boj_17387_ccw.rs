...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (min_l1_x, min_l1_y, max_l1_x, max_l1_y) =
        (min(x1, x2), min(y1, y2), max(x1, x2), max(y1, y2));
    let (min_l2_x, min_l2_y, max_l2_x, max_l2_y) =
        (min(x3, x4), min(y3, y4), max(x3, x4), max(y3, y4));

    // ccw 알고리즘으로 선분 l1과 점 l2의 두 끝점, 선분 l2와 l1의 두 끝점들의 위치관계를 탐색
