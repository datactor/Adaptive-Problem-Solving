...
이것이 Arc가 여러 스레드 간에 데이터를 공유하는 안전하고 신뢰할 수 있는
방법으로 간주되는 이유 중 하나이다.
...
다중 스레드 컨텍스트에서 가변 데이터에 대한 안전하고 효율적인 공유 액세스를 제공하기 위해
Arc가 Mutex 및 RwLock과 같은 다른 동기화 프리미티브와 함께 자주 사용된다는 점은 주목할 가치가 있다.
이러한 동기화 프리미티브를 Arc와 함께 사용하면 여러 스레드에서 안전하게 액세스할 수 있는
복잡한 데이터 구조를 만들 수 있다.