...
이 구현은 해시 테이블(HashMap)을 사용하여 노드를 저장하고 정렬된 벡터를 사용하여 키를 저장한다.  
...
(매번 해시링을 재조정한다는 것은 매 accountID마다 sort를 수행한다는 의미로 대규모 시스템에서 치명적이다.)
이는 대규모 시스템에서 비용이 많이 드는 작업이 될 수 있다. Rendezvous Hashing 또는 Jump Hashing과 같은 Consistent Hashing의
다른 구현은 이러한 단점 중 일부를 해결하기 위해 제안되었다.
...
## Limitations
Consistent Hashing에는 노드 실패 시 핫스팟 및 부하 불균형 가능성과 같은 몇 가지 risk가 있다.
또한 일부 응용 프로그램에서 필요할지도 모르는 strong consistency을 보장하지 않는다.  
